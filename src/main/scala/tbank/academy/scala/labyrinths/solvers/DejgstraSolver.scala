...
/**
 * @param start    Стартовая позиция в лабиринте
 * @param end      Конечная позиция в лабиринте
 * @param current  Текущее положение в лабиринте
 * @param previous Прошлое состояние
 * @param g        Расстояние от start
 */
private case class DijkstraState(start: Point, end: Point, current: Point, previous: Option[DijkstraState], g: Int)

/**
 * Алгоритм Дейстры аналогичен алгоритму 0-1 BFS на наших лабиринтах,
 * так что этот класс реализовывает второй алгоритм с сохранением следующих ячеек в стеке
 */
      ...
      case currentState :: pointsStackTail =>
        if (currentState.current == currentState.end)
          ...
          val nextStates = maze
            ...
            .map(point => DijkstraState(currentState.start, currentState.end, point, Some(currentState), currentState.g + 1))
